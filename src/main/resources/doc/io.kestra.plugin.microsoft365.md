...
Both `adaptivecards` tasks need DELEGATED (acting-as-user) authentication: set `username` and `password` instead of `clientSecret`/`pemCertificate` so the task signs in with Resource Owner Password Credentials (ROPC) as a real user or service account (one without MFA enforced, since ROPC has no interactive step). This also requires the Azure AD app registration to have "Allow public client flows" enabled, plus the delegated Graph permission for the operation: `ChannelMessage.Send` for `Send`, `Chat.ReadWrite` for `SendToUser`. App-only `clientSecret`/`pemCertificate` credentials are rejected by Microsoft Graph with an HTTP 403 for both tasks: sending a channel or chat message is not supported with application permissions alone.
